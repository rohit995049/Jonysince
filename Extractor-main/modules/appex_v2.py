...
async def appex_v2_txt(app, message, api, name):
    ...
    try:
        ...
        print(f"An error occurred: {str(e)}")
        return await message.reply_text("Please try again later. May be Password Wrong")
    hdr1 = {
        "Host": api,
        "Client-Service": "Appx",
        "Auth-Key": "appxapi",
    ...
    await message.reply_text("**✅ Login Successfully**")
    ...
    thread = threading.Thread(target=lambda: asyncio.run(course_content(app, api, message, raw_text2, batch_name, name, parent_Id, hdr1)))
    thread.start()
